/**
 * @file cipher-cc-mt.c
 * @brief generates keystream for chacha20 cipher
 * @date 2022-03-20
 */
#include "includes.h"
...
struct kq {
	u_char		keys[KQLEN][CHACHA_BLOCKLEN]; /* 8192 x 64B */
	u_char		ctr[CHACHA_BLOCKLEN]; /* 64B */
...
thread_loop(void *x)
{
	struct chachapoly_ctx *chacha_ctx;
	...
	pthread_t first_tid;
	int outlen;
	u_char mynull[CHACHA_BLOCK_SIZE];
	memset(&mynull, 0, CHACHA_BLOCK_SIZE);
	...
	if (!(chacha_ctx = EVP_CIPHER_CTX_new())) {
		logit("error cipher-cc-mt.c thread_loop function");
		exit(1);
	}
	...
	if (pthread_equal(pthread_self(), first_tid)) {
		...
		if (q->qstate == KQINIT) {
			...
			EVP_EncryptInit_ex(chcha_ctx, NULL, NULL, NULL, q->ctr);
			for (i = 0; i < KQLEN; i++) {
				...
				EVP_EncryptUpdate(chacha_ctx, q->keys[i], &outlen, mynull, CHACHA_BLOCKLEN);
				/* increment the counter */
				ssh_ctr_inc(q->ctr, CHACHA_BLOCKLEN);
			}
			ssh_ctr_add(q->ctr, KQLEN * (numkq - 1), CHACHA_BLOCKLEN);
			q->qstate = KQDRAINING;
    ...
    LIBCRYPTO_EVP_INL_TYPE len)
{
	...
	do { /* do until len is 0 */
		...
		destp.u +=  CHACHA_BLOCKLEN;
		srcp.u += CHACHA_BLOCKLEN;
    ...
    int enc)
{
	...
	if (c->state == (HAVE_KEY | HAVE_IV)) {
		...
		memcpy(c->q[0].ctr, c->chacha_counter, CHACHA_BLOCKLEN);
		...
		for (i = 1; i < numkq; i++) {
			memcpy(c->q[i].ctr, c->chacha_counter, CHACHA_BLOCKLEN);
			ssh_ctr_add(c->q[i].ctr, i * KQLEN, CHACHA_BLOCKLEN);
